...
assert(hasChoice(list, '203、旧名和当票', 'end_boss_lu'), `有外部陆小姐印证+学校当票时应显示陆小姐指认，实际 ${JSON.stringify(list)}`);
assert(hasChoice(list, '学校口径最能把事情压下去', 'end_boss_wu'), `有光华小学表层证据时应显示吴校长指认，实际 ${JSON.stringify(list)}`);
assertNoChoice(list, '赵先生', '没有赵先生线索时');

// 只有光华小学共同材料（当票、合影、学校异常）时，只能指向吴校长；不能因为每次都能拿到当票/合影就开启陆小姐。
reset({
  flags: {
    school_incomplete_closure: true,
    school_truth_partial_only: true,
...
assertNoChoice(list, '陆小姐', '仅学校共同材料时');
assertNoChoice(list, '赵先生', '仅学校共同材料时');
