...
for (const rel of requiredModules) {
  if (!exists(rel)) {
    console.error(`Chapter 3 Chen letter removal failed: required module missing: ${rel}`);
    process.exit(1);
  }
}
...
for (const rel of requiredModules) {
  if (!manifest.includes(rel)) {
    console.error(`Chapter 3 Chen letter removal failed: required module is not loaded: ${rel}`);
    process.exit(1);
  }
}
...
if (!found.length && missing.length === migratedNodes.length) {
  console.log(`Chapter 3 Chen letter removal check passed: no migrated Chen letter node definitions remain in ${storyRel}.`);
  process.exit(0);
}

if (found.length !== migratedNodes.length) {
  console.error('Chapter 3 Chen letter removal failed: partial removal detected.');
...
for (let i = 1; i < sortedRanges.length; i += 1) {
  if (sortedRanges[i].start < sortedRanges[i - 1].end) {
    console.error(`Chapter 3 Chen letter removal failed: overlapping ranges for ${sortedRanges[i - 1].id} and ${sortedRanges[i].id}`);
    process.exit(1);
  }
}

if (!write) {
  console.log(`Chapter 3 Chen letter removal dry-run: ${sortedRanges.length} node(s) can be removed from ${storyRel}.`);
...
console.log(`Removed ${sortedRanges.length} migrated Chapter 3 Chen letter node definition(s) from ${storyRel}.`);
