...
let dockChoices = rt.choicesOf('ch4_dock_escape');
assert(dockChoices.some(choice => choice.goto === 'ch4_dock_exit_assess'), '码头撤离应先进入黑车拦路判断节点');
rt.renderNode('ch4_dock_exit_assess');
dockChoices = rt.choicesOf('ch4_dock_exit_assess');
const hardConfront = dockChoices.find(choice => (choice.text || '').includes('车灯前'));
assert(hardConfront, '双救 + 一个便衣时应存在码头高风险亮证据选项');
...
const triageChoices = rt.choicesOf('ch4_hospital_triage').map(choice => choice.text || '');
assert(triageChoices.some(text => text.includes('送进病房')), '医院后门安置应提供先送病房选项');
assert(triageChoices.some(text => text.includes('守住医院后门')), '医院后门安置应提供守后门选项');
