...
assert(E.dockExposureScore() === 0, `完整证据谨慎路线暴露应为 0，实际 ${E.dockExposureScore()}`);
assert(E.dockDelayScore() === 3, `拿账房证据+慢行+躲箱应为拖延 3，实际 ${E.dockDelayScore()}`);
assert(E.dockHeatTier().key === 'low', `一个便衣平衡路线应允许证据拿全后仍为低风险，实际 ${JSON.stringify(E.dockHeatTier())}`);
assert(E.routeDockDeepByPressure() === 'ch4_dock_deep_dual', '证据拿全 + 平衡低调潜入应能救出两人');
...
assert(E.dockHeatTier().key === 'low', `单次触发守卫不应一票否决，实际 ${JSON.stringify(E.dockHeatTier())}`);
assert(E.routeDockDeepByPressure() === 'ch4_dock_deep_dual', '单次触发守卫但无其他拖延时，仍可救两人');
