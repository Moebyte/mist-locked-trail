...
assert(!texts.some(text => text.includes('正面压住傅启元')), '只有一个便衣时，不应显示正面压住傅启元');
assert(t.includes('end_dock_silenced'), '只有一个便衣时，硬质问应通往码头坏结局');

reset({
  flags: { sun_wait_support: true, sun_support_available: true, found_su_at_dock: true, found_yufang: true },
});
...
reset({
  flags: { sun_wait_support: true, sun_support_available: true, dock_sun_pressed_fu: true },
});
