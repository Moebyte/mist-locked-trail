...
test('只走巡捕房拿王巡官纸条，不应单独解锁薛华立路', () => {
  const rt = runtime({
    flags: { got_wang_note: true },
    clues: [{ name: '王巡官遗留纸条', desc: '测试占位' }],
    items: [{ name: '半张烟盒纸', desc: '测试占位' }],
  });
  const texts = choiceTexts(rt, 'ch2_police_wang');
  assert(texts.some(text => text.includes('光华小学')), '巡捕房拿王纸条后应能去光华小学');
  assert(!texts.some(text => text.includes('薛华立路') || text.includes('法租界')), '只走巡捕房不应解锁薛华立路');
});

test('先去大学拿到法租界地图后，再去巡捕房，仍可显示薛华立路', () => {
  const rt = runtime({
    flags: { got_wang_note: true },
    clues: [
      { name: '王巡官遗留纸条', desc: '测试占位' },
      { name: '法租界地图', desc: '测试占位' },
    ],
    items: [{ name: '半张烟盒纸', desc: '测试占位' }],
  });
  const texts = choiceTexts(rt, 'ch2_police_wang');
  assert(texts.some(text => text.includes('薛华立路') || text.includes('法租界')), '大学解锁薛华立路后，巡捕房回流菜单应保留薛华立路');
});
