...
reset({
  ...
  items: [{ name: '卷宗摘抄', desc: '' }, { name: '陈明远的信', desc: '信的开头是“晚亭吾爱”。' }, { name: '苏晚亭的遗书', desc: '' }],
});
...
assert(hasChoice('ch3_wrapup', '陈明远的信和苏晚亭的遗书'), '坏路线回访周怀安入口应明确两封信');
...
assert(rt.E.hasItem('陈明远的信'), '坏路线读陈明远的信后，应获得陈明远的信');
assert(rt.E.hasItem('苏晚亭的遗书'), '坏路线读陈明远的信后，应获得苏晚亭的遗书');
assert(!rt.E.hasItem('苏晚亭的伪造遗书'), '玩家侧不应获得名为“伪造遗书”的物品');
assert(rt.E.hasClue('苏晚亭的遗书'), '坏路线读陈明远的信后，应记录苏晚亭的遗书线索');
assert(!rt.E.hasClue('苏晚亭伪造遗书'), '玩家侧线索不应直接写明遗书伪造');
assert(!nodeText('ch3_chen_letter').includes('伪造'), '陈明远信节点不应直接剧透遗书是伪造的');
assert(nodeText('ch3_chen_letter').includes('不能判断这封遗书是真是假'), '陈明远信节点应保持玩家视角的不确定');

reset({
  flags: { got_case_file: true, asked_about_chen: true, chen_su_link: true, got_chen_evidence: true, read_letter: true },
  clues: [{ name: '光华小学事件', desc: '' }, { name: '陈明远的信', desc: '' }, { name: '苏晚亭的遗书', desc: '' }],
  items: [{ name: '陈明远的信', desc: '信的开头是“晚亭吾爱”。' }, { name: '苏晚亭的遗书', desc: '' }],
});
...
assert(nodeText('end_zhou_chen_letter').includes('这不是殉情'), '吾爱晚亭结局应否定殉情叙事');
...
rt.renderNode('ch3_chen_letter');
assert(!rt.E.hasItem('苏晚亭的遗书'), '前置线索充足路线读陈明远的信时，不应获得苏晚亭的遗书');
...
const fullRouteNote = present('ch4_revisit_zhou', '苏晚亭的遗书');
assert(!fullRouteNote, '完整证据路线下，即使异常持有遗书也不应触发吾爱晚亭彩蛋');
