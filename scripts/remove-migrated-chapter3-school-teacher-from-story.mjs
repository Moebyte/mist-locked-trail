...
for (const rel of requiredModules) {
  if (!exists(rel)) {
    console.error(`Chapter 3 school teacher removal failed: required module missing: ${rel}`);
    process.exit(1);
  }
}
...
for (const rel of requiredModules) {
  if (!manifest.includes(rel)) {
    console.error(`Chapter 3 school teacher removal failed: required module is not loaded: ${rel}`);
    process.exit(1);
  }
}

const moduleSource = read('src/story-chapters/chapter-3-guanghua.js');
for (const id of migratedNodes) {
  if (!moduleSource.includes(id)) {
    console.error(`Chapter 3 school teacher removal failed: migrated node is not present in chapter module: ${id}`);
    process.exit(1);
  }
}

const contractSource = read('src/story-chapters/chapter-3-guanghua-contract.js');
for (const id of migratedNodes) {
  if (!contractSource.includes(id)) {
    console.error(`Chapter 3 school teacher removal failed: migrated node is not present in chapter contract: ${id}`);
    process.exit(1);
  }
}
...
if (!found.length && missing.length === migratedNodes.length) {
  console.log(`Chapter 3 school teacher removal check passed: no migrated school teacher node definitions remain in ${storyRel}.`);
  process.exit(0);
}

if (found.length !== migratedNodes.length) {
  console.error('Chapter 3 school teacher removal failed: partial removal detected.');
...
for (let i = 1; i < sortedRanges.length; i += 1) {
  if (sortedRanges[i].start < sortedRanges[i - 1].end) {
    console.error(`Chapter 3 school teacher removal failed: overlapping ranges for ${sortedRanges[i - 1].id} and ${sortedRanges[i].id}`);
    process.exit(1);
  }
}

if (!write) {
  console.log(`Chapter 3 school teacher removal dry-run: ${sortedRanges.length} node(s) can be removed from ${storyRel}.`);
...
console.log(`Removed ${sortedRanges.length} migrated Chapter 3 school teacher node definition(s) from ${storyRel}.`);
