...
function policeToSchoolState(overrides = {}) {
  return {
    flags: { got_case_file: true, ...(overrides.flags || {}) },
    items: [{ name: '卷宗摘抄', desc: '' }, ...(overrides.items || [])],
    clues: [{ name: '光华小学事件', desc: '' }, ...(overrides.clues || [])],
...
// 专属坏路线：接案后先去巡捕房拿卷宗，再直接去光华小学，读陈明远信时应变成被整理过的信封，得到残信 + 疑似遗书。
reset(policeToSchoolState());
let text = textOf('ch3_chen_letter');
assert(text.includes('信封边缘压得很平'), `巡捕房→光华坏路线应出现被重新封过的迹象，实际：${text}`);
assert(text.includes('信纸下半截也不见了'), `巡捕房→光华坏路线陈明远信应是残信，实际：${text}`);
assert(text.includes('这个信封究竟保留了什么，又拿走了什么'), `巡捕房→光华坏路线应强调信息被整理过但不能当场识破，实际：${text}`);
runEffect('ch3_chen_letter');
assert(E.getFlag('chen_letter_packet_altered'), '巡捕房→光华坏路线应设置 chen_letter_packet_altered');
assert(E.hasItem('陈明远残信'), '巡捕房→光华坏路线应获得陈明远残信');
assert(E.hasItem('苏晚亭疑似遗书'), '巡捕房→光华坏路线应获得苏晚亭疑似遗书');
assert(E.hasClue('陈明远残信'), '巡捕房→光华坏路线应加入陈明远残信线索');
assert(E.hasClue('苏晚亭疑似遗书'), '巡捕房→光华坏路线应加入苏晚亭疑似遗书线索');
...
// 泛化坏路线但不是“巡捕房→光华”时，也不应获得吾爱晚亭专属信封。
reset({
...
assert(!text.includes('信封边缘压得很平'), `非直达光华路线不应出现被整理过的信封，实际：${text}`);
runEffect('ch3_chen_letter');
assert(!E.hasItem('陈明远残信'), '非直达光华路线不应获得陈明远残信');
assert(!E.hasItem('苏晚亭疑似遗书'), '非直达光华路线不应获得苏晚亭疑似遗书');
