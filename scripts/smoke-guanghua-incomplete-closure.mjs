...
assert(text.includes('话还留着半截'), `缺证时吴校长质询页应使用叙事阻滞提示，实际：${text}`);
assert(text.includes('陈明远死前究竟被谁逼到墙角') && text.includes('苏晚亭到底是主动追查'), `缺证提示应写成叙事空白，实际：${text}`);
assert(text.includes('学校背后的人'), `缺证提示应使用非剧透表达，实际：${text}`);
...
assert(hasChoice(list, '更容易被写进案卷', 'ch3_school_incomplete_closure'), `缺证时应提供叙事化表层结案入口，实际 ${JSON.stringify(list)}`);
...
assert(hasChoice(list, '更容易被写进案卷', 'ch4_conclusion'), `表层结案应回到结案整理，实际 ${JSON.stringify(list)}`);
...
assert(text.includes('雾只散到这里'), `表层结案后 wrapup 应以叙事口吻提示只到表层，实际：${text}`);
...
assert(!hasChoice(list, '更容易被写进案卷', 'ch3_school_incomplete_closure'), `三证齐全时不应提供表层结案入口，实际 ${JSON.stringify(list)}`);
