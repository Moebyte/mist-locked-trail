// ===== 苏家信任门槛 =====
// 目标：苏家不只是补叙。救苏晚亭时，需要“向苏母出示照片”形成的托付凭据。
// 没有这一步，仍可找到苏晚亭，但无法在码头局势中成功带走她，完美救援降档。
(function installSuHomeTrustGate() {
  function applySuHomeTrustGate() {
    ...
    function hasSuHomeTrustProof() {
      return E.getFlag('shown_photo_to_mother') || E.hasClue('苏母认出照片');
    }
    ...
    if (nodes.ch4_dock_escape_finish && !nodes.ch4_dock_escape_finish.__suHomeTrustGatePatched) {
      ...
      nodes.ch4_dock_escape_finish.effect = function (state) {
        ...
        if (foundSu && !hasSuHomeTrustProof()) {
          ...
          E.addClue('苏晚亭救援失之交臂', '你在福生仓见到了苏晚亭，但缺少苏母认出照片后的托付凭据；码头混乱中，她被傅启元的人转走。');
          return;
        }
      ...
      nodes.ch4_dock_escape_finish.text = function (state) {
        if (E.getFlag('found_su_at_dock') && !hasSuHomeTrustProof()) {
          return `黄包车的铃铛在深夜街道上响起。<br><br>沈玉芳蜷在车座一角，手指死死攥着你的衣袖。苏晚亭曾经就在你背后不远处，可码头乱起来的那一刻，她没有跟上来。<br><br>她太虚弱，也太警惕。你说自己是周怀安请来的侦探，可她只是看着你，像隔着一层很厚的雾。<br><br>如果你去过苏家，把那张苏晚亭的照片拿给苏母看，带着母亲亲口托付的那句话回来，也许她会相信你。<br><br>可你没有。<br><br>傅启元的人趁乱把她重新拖上车。你只来得及抢回沈玉芳，和苏晚亭曾经还活着的证明。`;
        }
    ...
    if (nodes.ch4_dock_who_dual && !nodes.ch4_dock_who_dual.__suHomeTrustHintPatched) {
      ...
      nodes.ch4_dock_who_dual.text = function (state) {
        ...
        if (!hasSuHomeTrustProof()) {
          return `${base}<br><br><span class="sys">苏晚亭看向你的眼神仍然很远。你手里有证件，有线索，却没有苏母认出照片后的托付凭据。她不知道你是不是又一个来“带她走”的人。</span>`;
        }
