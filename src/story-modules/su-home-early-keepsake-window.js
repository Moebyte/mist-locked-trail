...
// 普通问询仍显示完整叙事段落，不再把苏家写成“已问到”摘要清单。
(function installSuHomeEarlyKeepsakeWindow() {
  function applySuHomeEarlyKeepsakeWindow() {
    ...
    function setHomeDetail(detail) {
      E.setFlag?.('home_last_detail', detail);
    }
    ...
    function detailText() {
      ...
      if (detail === 'showphoto') return keepsakeText();
      return baseHomeText();
    }

    if (nodes.ch2_home && !nodes.ch2_home.__earlyKeepsakeWindowPatched) {
      nodes.ch2_home.text = function () {
        if (E.getFlag?.('su_home_show_early_visit_scene')) {
          E.state.flags.su_home_show_early_visit_scene = false;
          return earlyVisitText();
        }
        if (E.getFlag?.('su_home_show_keepsake_scene')) {
          E.state.flags.su_home_show_keepsake_scene = false;
          return keepsakeText();
        }
        return detailText();
      };
      ...
      nodes.ch2_home.choices = function () {
        ...
        if (!E.hasClue?.('母亲证词')) opts.push({
          ...
          effect: () => {
            ...
            if (!hasRecentSuPhoto()) {
              markEarlyVisit();
              E.setFlag?.('su_home_show_early_visit_scene', true);
            }
          },
          goto: 'ch2_home'
        });
        if (!E.getFlag?.('asked_photo')) opts.push({
          text: '🖼️ 细看墙上的全家福',
          effect: () => {
        ...
        if (E.getFlag?.('asked_photo') && !E.getFlag?.('asked_mother_photo')) opts.push({
          text: '💬 问苏母——照片里被裁掉的是谁',
          effect: () => {
            E.addClue?.('表哥', '照片上裁掉的人是苏晚亭的表哥；苏母不愿多谈');
            E.addClue?.('苏家旧伤', '墙上全家福被裁掉的人只是苏家不愿多谈的旧关系，不像当前失踪案的直接线索。');
            E.setFlag?.('asked_mother_photo', true);
            setHomeDetail('ask_photo');
          },
          goto: 'ch2_home'
        });
        if (hasRecentSuPhoto() && !E.getFlag?.('shown_photo_to_mother')) opts.push({
          text: E.getFlag?.('su_home_early_without_photo') ? '🖼️ 把苏晚亭留下的照片放到苏母面前' : '🖼️ 拿出苏晚亭的照片给苏母看',
          effect: () => {
            giveKeepsake();
            setHomeDetail('showphoto');
            E.setFlag?.('su_home_show_keepsake_scene', true);
          },
          goto: 'ch2_home'
        });
