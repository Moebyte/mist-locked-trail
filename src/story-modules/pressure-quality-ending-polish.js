// ===== 压力 / 案件质量 / 真隐藏结局收束 =====
// 目标：压力决定是否赶得上救人；案件质量决定能否说清真相；真·隐藏结局必须救出苏晚亭本人。
...
(function installPressureQualityEndingPolish() {
  function applyPressureQualityEndingPolish() {
    ...
    const oldAddHeat = typeof E.addHeat === 'function' ? E.addHeat.bind(E) : null;
    ...
    E.v07InvestigationQuality = function () {
      ...
      if (!this.getFlag('rescued_su')) {
        q.score = Math.min(q.score, 9);
        q.reasons.push('苏晚亭没有被救出，不能达到真·隐藏结局质量');
      }
    ...
    if (nodes.end_conspiracy_detail && !nodes.end_conspiracy_detail.__trueHiddenTextPatched) {
      nodes.end_conspiracy_detail.title = '结局 · 雨夜灯火';
      nodes.end_conspiracy_detail.effect = () => {
        E.addClue('真隐藏结局已解锁', '你完成光华小学三证物闭环，并在福生仓救出了苏晚亭。');
      };
      nodes.end_conspiracy_detail.text = () => `所有的碎片都拼上了。<br><br>陈明远发现光华小学的管制药品走私——被灭口。<br>沈玉芳从他那里知道了一部分真相——被关进福生仓。<br>陆念薇是中间人——她被杭州旧案捏住脖子，不是主谋。<br>傅启元在码头亲自现身——蓝封公文夹终于有了主人。<br><br>而最重要的是：苏晚亭没有只剩下学生证、手表或半张字条。<br><br>她被你从福生仓带了出来。<br><br>在教会医院醒来后，她亲手写下了自己在福生仓听见的名字：傅启元。她的证词、沈玉芳的证词、光华货运单、清场指令和吴校长三证物质询，终于把那张看不见的网钉在了纸面上。<br><br>你没有只写一份报案材料。你写了三份：一份交给老孙，一份寄给《申报》，一份锁进银行保险柜。<br><br>三天后，《申报》头版刊出报道：《光华小学教具箱暗藏管制药品，法租界码头仓库涉非法转运》。<br><br>报道第一次点出了傅启元的名字。<br><br>又过了三天，福生仓被查封。傅启元以“协助调查”的名义被带走。公董局没有承认任何事，但他们也没能让这件事完全消失。<br><br>一个月后，你收到一封信。信上只有一行字：<br><br><span class="sys">“沈先生，谢谢你先找人，而不是先找凶手。——苏晚亭”</span><br><br>窗外又下雨了。你泡了一壶新茶。<br><br>民国三十七年的冬天，比往年来得都晚一些。但终究是来了。<br><br><div style="color:#666;font-style:italic;margin-top:20px">—— 结局九 · 雨夜灯火（真·隐藏结局）——</div>`;
      nodes.end_conspiracy_detail.__trueHiddenTextPatched = true;
    }
