...
function applyResponsiveStoryUI() {
  ...
  function splitSceneHtml(html) {
    // 只在显式标记 <!--PAGE_BREAK--> 处分页，不按字数自动截断
    const hasBreak = /<!--PAGE_BREAK-->/i.test(html);
    if (!hasBreak) return [html];
    return html
      .split(/<!--PAGE_BREAK-->/i)
      .map(s => s.trim())
      .filter(Boolean);
  ...
  E.scroll = function () {
    if (document.body.classList.contains('game-active') && this.sceneEl) {
      // 重置内部滚动位置到顶部
      if (this.textEl) this.textEl.scrollTop = 0;
      if (this.sceneEl) this.sceneEl.scrollTop = 0;
